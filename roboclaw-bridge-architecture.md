# roboclaw-bridge 架构设计文档

> 本文档记录了 roboclaw-bridge 的完整设计思路、技术决策、踩过的坑和解决方案。目的是让团队成员快速理解这套系统的来龙去脉。

---

## 1. 核心问题：LLM 怎么控制机器人？

我们的 roboclaw 是一个 LLM agent 框架。我们要让 LLM 能够：
...
关键矛盾：roboclaw 运行在 Python 3.11（roboclaw-dev conda env），而 lerobot（机器人 SDK）需要 Python 3.10（lerobot conda env）。它们不能在同一个进程里。
...
MCP（Model Context Protocol）是一个 JSON-RPC over stdio 的协议。roboclaw 已经原生支持 MCP——外部进程通过 stdin/stdout 暴露 tools，roboclaw 把它们包装成 `MCPToolWrapper`，LLM 完全无法区分 MCP tool 和原生 tool。
...
```
roboclaw (LLM agent, roboclaw-dev env, Python 3.11)
    │ stdio JSON-RPC (MCP protocol)
    ├──────────────────────────┐
    ▼                          ▼
robot-bridge                 sim-bridge
(lerobot env, Python 3.10)   (roboclaw-dev env, Python 3.11)
    │                          │
...
roboclaw 启动时根据 `~/.roboclaw/config.json` 中的 `mcp_servers` 配置，spawn 两个子进程：
- `robot-bridge` — 使用 lerobot 环境的 Python 解释器
- `sim-bridge` — 使用 roboclaw-dev 环境的 Python 解释器

每个 bridge 暴露一组 MCP tools，roboclaw 自动发现并注册。
  ...
  "tools": {
    "mcp_servers": {
      "robot": {
        ...
        "env": {"ROBOCLAW_WORKSPACE": "~/.roboclaw/workspace"},
        "tool_timeout": 60
      },
      "sim": {
        "command": "/path/to/conda/envs/roboclaw-dev/bin/python",
...
class DriverLoader:
    def load(self, name, reload=False):
        ...
        spec = importlib.util.spec_from_file_location(f"_roboclaw_driver_{name}", path)
...
核心思路：用 `importlib` 从 `~/.roboclaw/workspace/drivers/{name}.py` 动态加载。支持 reload（agent 修改 driver 后重新加载）。每个 driver 文件必须导出一个 `Driver` class。
...
Driver 是纯 Python 文件，存放在 `~/.roboclaw/workspace/drivers/`。这个路径是 agent 的工作区，LLM 可以直接读写文件。
...
2. 用 `write_file` 写一个新 driver 到 `~/.roboclaw/workspace/drivers/`
3. `load_driver` 加载
...
问题：如果 roboclaw 启动时就初始化 PhysicsEngine，GUI 窗口会立即弹出——即使用户还没用到仿真。
...
- Python/MCP 写 sys.stdout → 进入真实 stdout → roboclaw 收到干净的 JSON-RPC
...
**解决：** 修改了 roboclaw 的 `roboclaw/agent/tools/mcp.py`，让 MCP 客户端传递显示相关的环境变量：
...
`~/.roboclaw/workspace/skills/robot/SKILL.md`
...
6. **工具名称冗余** — MCP server 名 `sim` + tool 名 `sim_load_robot` -> roboclaw 注册为 `mcp_sim_sim_load_robot`，双重 `sim_` 很丑。
...
- [ ] `roboclaw init-robot` CLI 命令 — 一键创建 conda env、安装 bridge、写 config
...
```bash
# 解压
unzip roboclaw-bridge.zip

# 仿真环境（最简）
conda activate roboclaw-dev  # 或任何 Python >= 3.10
cd roboclaw-bridge
...
1. 把 roboclaw-bridge 装到对应 conda 环境
2. 编辑 `~/.roboclaw/config.json` 添加 mcp_servers（见第 2.3 节）
3. 启动 roboclaw，应该能看到 bridge 的 tools 被注册
4. LLM 就可以用了
...
| `~/.roboclaw/config.json` | roboclaw MCP 服务器配置 |
| `~/.roboclaw/workspace/drivers/` | Agent 工作区 driver 目录 |
| `~/.roboclaw/workspace/skills/robot/SKILL.md` | Always-on LLM skill |
| `roboclaw/agent/tools/mcp.py` | roboclaw 端 DISPLAY 传递 patch |
