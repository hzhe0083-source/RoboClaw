...
MCP (Model Context Protocol) bridge connecting the [roboclaw](https://github.com/wavecolab/roboclaw) AI agent to real robots and simulations. A single package providing two bridge servers: one for real hardware, one for PyBullet simulation.

## Architecture

```
roboclaw (LLM agent, roboclaw-dev env)
    │ stdio JSON-RPC (MCP protocol)
    ├──────────────────────┐
    ▼                      ▼
robot-bridge             sim-bridge
(lerobot env)            (roboclaw-dev env)
    │                      │
...
- **Cross-environment**: Bridges run in the robot SDK's conda env (e.g. `lerobot` Python 3.10). RoboClaw runs in its own env. MCP stdio connects them.
...
```bash
conda activate roboclaw-dev  # or any Python >= 3.10 env

# Install with simulation dependencies
pip install -e "/path/to/roboclaw-bridge[sim,dev]"
...
conda activate roboclaw-dev
pip install -e "/path/to/roboclaw-bridge[sim]"

# 2. Install in the robot SDK environment (no sim deps needed)
conda activate lerobot
pip install -e /path/to/roboclaw-bridge

# 3. Copy reference drivers to workspace
mkdir -p ~/.roboclaw/workspace/drivers
cp examples/drivers/so100_real.py ~/.roboclaw/workspace/drivers/
cp examples/drivers/so100_sim.py ~/.roboclaw/workspace/drivers/
# Edit so100_sim.py to set your URDF_PATH
```

### Configure roboclaw

Add to `~/.roboclaw/config.json`:
...
> **DISPLAY passthrough** is automatic — roboclaw's MCP client passes `DISPLAY` and related env vars to child processes.
...
# Sim bridge
/path/to/envs/roboclaw-dev/bin/python -m sim_bridge.server  # Ctrl+C to exit
```
...
| `load_driver` | Load driver from `~/.roboclaw/workspace/drivers/{name}.py` |
...
Drivers are Python files in `~/.roboclaw/workspace/drivers/`. Each exports a `Driver` class:
...
6. **`mcp_sim_sim_*` name stutter** — MCP server name `sim` + tool name `sim_load_robot` produces `mcp_sim_sim_load_robot` in roboclaw.
7. **No driver dependency declaration** — Drivers can't specify their pip requirements.

### Roadmap

- [ ] `roboclaw init-robot` CLI — one-command setup (create envs, install bridges, write config)
