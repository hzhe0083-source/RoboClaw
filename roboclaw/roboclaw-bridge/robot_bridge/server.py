...
DEFAULT_WORKSPACE = Path.home() / ".roboclaw" / "workspace"
...
def create_server(workspace: Path | None = None) -> FastMCP:
    ...
    mcp = FastMCP("roboclaw-robot-bridge")
...
def main():
    """Entry point for CLI: roboclaw-robot-bridge"""
    import argparse
    import os

    parser = argparse.ArgumentParser(description="roboclaw robot bridge MCP server")
    parser.add_argument("--workspace", type=str, default=os.environ.get("ROBOCLAW_WORKSPACE", ""))
